...
export default function SearchClient() {
  ...
  async function search(e: React.FormEvent) {
    ...
    if (!q.trim()) return;
    setLoading(true);
    const res = await fetch(`${PUBLIC_API_URL}/rawg/search?q=${encodeURIComponent(q)}`, {
      credentials: "include",
    });
        ...
        {results.map((g) => (
          <li key={g.id} className="overflow-hidden rounded-lg border border-neutral-800 bg-neutral-900">
            ...
            <div className="space-y-1 p-3">
              ...
              <button onClick={() => setPicked(g)} className="mt-2 rounded bg-emerald-600 px-3 py-1 text-xs">
                Añadir a biblioteca
              </button>
            </div>
          </li>
